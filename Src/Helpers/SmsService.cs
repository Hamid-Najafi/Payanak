...
namespace Backend.Helpers
{
    public class SmsService
    {
                                                        ...
                                                        VwContact user)
        {
            ...
            try
            {
                var res = new SendService.SendSoapClient(SendService.SendSoapClient.EndpointConfiguration.SendSoap);
                        ...
                        while (first < numbers.Count)
                        {
                            ...
                            using (var _transaction = context.Database.BeginTransaction())
                            {
                                var nums = numbers.Skip(first)
                                                ...
                                                .Select(a => a.Replace("-", ""))
                                                .ToList();
                                var rectIds = await res.SendSimpleSMSAsync(username,
                                                   password,
                                                   nums.ToArray(),
                                                   sendNumber,
                                                   body,
                                ...
                                context.SentInfo.Add(new SentInfo
                                {
                                    ...
                                    RectIds = SmsService.NumberListToString(rectIds),
                                    SendNumber = sendNumber,
                                    UserId = user.Id,
                                    SentDate = DateTime.UtcNow,
        ...
        public static async Task<string[]> sendSmsToGroups(string username,
                                                        string password,
                                                        string sendNumber,
                                                        List<long> groupIds,
                                                        string body,
            ...
            try
            {
                ...
                using (var context = new SmsPanelDbContext())
                {
                    ...
                    try
                    {
                        ...
                        taskRes.Status.Add(new ResponseStatusModel(ResponseStatus.ok));
                        taskRes.Header = "فرآیند ارسال پیام به گروه با عنوان \'" + header + "\'";
                        TaskHelper.SetResult(guid, taskRes);
                        ...
                        var numbers = fixNumbers(userNums);
                        var hasFilterWord = await SmsService.HasFilteredWords(username,
                                                                        password,
                                                                        body);
                        if (hasFilterWord)
                        {
                            ...
                            context.SentInfo.Add(new SentInfo
                            {
                                Body = body,
                                Deliveries = null,
                                GroupIds = SmsService.NumberListToString(groupIds.Select(a => a.ToString()).ToArray()),
                                Header = header,
                                Kind = 1, // ارسال به لیست گروه
                                Numbers = SmsService.NumberListToString(numbers.Select(a => a.Replace("-", "")).ToArray()),
                                RectIds = null,
                                SendNumber = sendNumber,
                                UserId = user.Id,
                                SentDate = DateTime.UtcNow,
                                Status = 1, // ارسال ناموفق
                                CalculatedCount = 0,
                                Count = 0,
                                Price = 0
                            });
                        ...
                        while (first < numbers.Count)
                        {
                            using (var _transaction = context.Database.BeginTransaction())
                            {
                                ...
                                var rectIds = await res.SendSimpleSMSAsync(username,
                                                   ...
                                                   false);
                                first += count;
                                taskRes.Percent = (int)(first / (numbers.Count * 1.0f));
                                TaskHelper.SetResult(guid, taskRes);
                                allRectIds.AddRange(rectIds);
                                var sumSmsInWhile = SmsCounter(nums.Count, body);
                                var sumPriceInWhile = sumSmsInWhile * (int)Math.Ceiling((float)setting.SmsPrice.Value * (SmsDiscount / 100.0f) * (userDiscount / 100.0f));
