...
export class AddBusinessCardModelComponent implements OnInit {
  ...
  loadUserGroups() {
    ...
    this.smsService.getUserGroupsForAssign(owner.id).subscribe(
      res => {
        if (res && res.status && res.status.length === 1 && res.status[0].status === 200) {
          this.contacts = res.result;
    ...
    if (owner && event && event.id === '-1') {
      ...
      ref.result.then(res => {
        if ((typeof (res)) !== 'string') {
          this.smsService.createTemplateForUser(owner.id, res).subscribe(
            res => {
  ...
  loadNumberAndTemplate(event) {
    ...
    this.smsService.getUserNumbersForAssign(owner.id).subscribe(
      res => {
        if (res && res.status && res.status.length === 1 && res.status[0].status === 200) {
          this.contacts = res.result;
          ...
          this.smsService.getUserTemplatesForAssign(owner.id).subscribe(
            res => {
              if (res && res.status && res.status.length === 1 && res.status[0].status === 200) {
                this.contacts = res.result;
