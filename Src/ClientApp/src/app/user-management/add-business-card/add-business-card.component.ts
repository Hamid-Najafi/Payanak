...
export class AddBusinessCardComponent implements OnInit, OnDestroy {
  ...
  loadData() {
    ...
    this.loading = true;
    this.smsService.getAllBusinessCards(this.queryParam).pipe(
      take(1))
      .subscribe(
        res => {
  ...
  deactivate(businessCard: BusinessCardModel) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.loading = true;
        this.smsService.deactivateBusinessCard(businessCard.id).subscribe(
          res => {
            this.loading = false;
  ...
  block(businessCard: BusinessCardModel) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.loading = true;
        this.smsService.blockBusinessCard(businessCard).subscribe(
          res => {
            this.loading = false;
            this.successAndLoad(res);
          },
  ...
  delete(businessCard: BusinessCardModel) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.loading = true;
        this.smsService.deleteBusinessCard(businessCard.id).subscribe(
          res => {
            this.loading = false;
            this.successAndLoad(res);
          },
  ...
  lunchAddCard() {
    ...
    ref.result.then(res => {
      if ((typeof (res)) !== 'string') {
        this.loading = false;
        this.smsService.addBusinessCard(res).subscribe(
          res => {
            this.loading = false;
            this.successAndLoad(res);
          }
  ...
  edit(businessCard: BusinessCardModel) {
    ...
    ref.result.then(res => {
      if ((typeof (res)) !== 'string') {
        this.loading = true;
        this.smsService.editBusinessCard(res).subscribe(
          res => {
            this.loading = false;
            this.successAndLoad(res);
          }
