...
export class PersonalTemplateComponent implements OnInit, OnDestroy {
  ...
  loadData() {
    ...
    this.loading = true;
    this.smsService.getUserTemplates(this.queryParam).pipe(
      take(1))
      .subscribe(
        res => {
  ...
  lunchAddTemplate() {
    ...
    ref.result.then(res => {
      if ((typeof (res)) !== 'string') {
        this.loading = true;
        this.smsService.addTemplate(res).subscribe(
          res => {
            this.loading = false;
  ...
  delete(template: TemplateModel) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.loading = true;
        this.smsService.deleteTemplate(template.id).subscribe(
          res => {
            this.loading = false;
            this.successAndLoad(res);
          },
  ...
  edit(template: TemplateModel) {
    ...
    ref.result.then(res => {
      if ((typeof (res)) !== 'string') {
        this.loading = true;
        this.smsService.editTemplate(res).subscribe(
          res => {
            this.loading = false;
            this.successAndLoad(res);
          },
