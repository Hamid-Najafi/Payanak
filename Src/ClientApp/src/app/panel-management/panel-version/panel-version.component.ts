...
export class PanelVersionComponent implements OnInit, OnDestroy {
  ...
  loadData() {
    ...
    this.loading = true;
    this.smsService.getPanelVersions(this.queryParam).pipe(
      take(1))
      .subscribe(
        res => {
  ...
  delete(version: PanelVersionModel) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.loading = true;
        this.smsService.deletePanelVersion(version.id).subscribe(
          res => {
            this.loading = false;
  ...
  lunchAddPanelVersion() {
    ...
    ref.result.then(res => {
      if ((typeof (res)) !== 'string') {
        this.loading = true;
        this.smsService.addPanelVersion(res.versionModel, res.files[0]).subscribe(
          res => {
            this.loading = false;
            this.successAndLoad(res);
          }
